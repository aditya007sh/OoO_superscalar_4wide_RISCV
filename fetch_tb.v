// ============================================================
// LOAD/STORE HAZARD TESTBENCH - compact version
// 12 instructions, 2 addresses, all hazards covered
// ============================================================
`timescale 1ns / 1ps

module tb_loadstore_test;

    reg clk, rst;
    always #1 clk = ~clk;  // 2ns period (fast sim clock)
    ...
    begin
        ...
        if (actual === expected) begin
            ...
            $display("  x%-2d = %-12d (0x%08h)  FAIL  (expected %0d)  %0s",
                     rn, actual, actual, expected, label);
            fail_cnt = fail_cnt + 1;
        end
    end
    endtask
    ...
    task check_mem;
        ...
        reg [31:0] actual;
    begin
        actual = dut.u_issue.u_mem.mem[widx];
        if (actual === expected) begin
            $display("  mem[%0d] = %-12d (0x%08h)  PASS  %0s", widx, actual, actual, label);
            pass_cnt = pass_cnt + 1;
        end else begin
            $display("  mem[%0d] = %-12d (0x%08h)  FAIL  (expected %0d)  %0s",
                     widx, actual, actual, expected, label);
            fail_cnt = fail_cnt + 1;
        end
    end
    endtask
    ...
    initial begin
        ...
        // 240 cycles at 2ns = 480ns (fits in 1000ns)
        repeat (100) @(posedge clk);

        // ARF dump
        $display("############################################################");
        $display("# ARF REGISTER VALUES");
        $display("############################################################");
        for (i = 0; i < 16; i = i + 1)
        ...
        $display("");
        $display("############################################################");
        $display("# MEMORY (addr 0 and 4)");
        $display("############################################################");
        $display("  mem[0] (addr 0) = %-12d (0x%08h)", dut.u_issue.u_mem.mem[0], dut.u_issue.u_mem.mem[0]);
        $display("  mem[1] (addr 4) = %-12d (0x%08h)", dut.u_issue.u_mem.mem[1], dut.u_issue.u_mem.mem[1]);
        ...
        $display("  --- Basic SW+LW + store-to-load fwd + load-use ---");
        check_mem(0,  32'd99,  "mem[0] final=99 (10->20->99)");
        check_mem(1,  32'd20,  "mem[1]=20");
        check_reg(3,  32'd10,  "LW x3, 0(x0)=10 (fwd from 1st store)");
        check_reg(4,  32'd20,  "ADD x4=x3+x1=10+10=20 (load-use)");

        $display("  --- Memory WAW: overwrite addr 0 ---");
        check_reg(5,  32'd20,  "LW x5, 0(x0)=20 (after WAW overwrite)");

        $display("  --- Back-to-back S+S+L ---");
        check_reg(6,  32'd99,  "ADDI x6=99");
        check_reg(7,  32'd99,  "LW x7, 0(x0)=99 (latest of 3 stores)");
        check_reg(8,  32'd20,  "LW x8, 4(x0)=20 (different addr)");
