// ============================================================
// BRANCH TESTBENCH - 3 mispredictions + 1 correct prediction
// Checks: flush, register freeing (freelist), post-flush RAW deps
// ============================================================
`timescale 1ns / 1ps
...
module tb_branch_test;
    ...
    // ================================================================
    // COUNT FREE REGS IN FREELIST
    // ================================================================
    function integer count_free;
        input [63:0] bmap;
        integer k;
    begin
        count_free = 0;
        for (k = 0; k < 64; k = k + 1)
            count_free = count_free + bmap[k];
    end
    endfunction
    ...
    begin
        $display("========================================");
        $display("CYCLE %-3d  (t=%0t)", cycle, $time);
        $display("========================================");
        $display("  PC=0x%08h  ROB: h=%0d t=%0d cnt=%0d  Free_PRFs=%0d",
            dbg_pc_current, dbg_rob_head, dbg_rob_tail, dbg_rob_count,
            count_free(dbg_freelist_bitmap));

        if (cdb_fu0_valid) $display("  CDB0: rob=%0d  rd->p%-2d  val=%-5d (0x%08h)", cdb_fu0_rob_idx, cdb_fu0_phys_reg, cdb_fu0_result, cdb_fu0_result);
        if (cdb_fu1_valid) $display("  CDB1: rob=%0d  rd->p%-2d  val=%-5d (0x%08h)", cdb_fu1_rob_idx, cdb_fu1_phys_reg, cdb_fu1_result, cdb_fu1_result);
        if (cdb_fu2_valid) $display("  CDB2: rob=%0d  rd->p%-2d  val=%-5d (0x%08h)", cdb_fu2_rob_idx, cdb_fu2_phys_reg, cdb_fu2_result, cdb_fu2_result);
        if (cdb_fu3_valid) $display("  CDB3: rob=%0d  rd->p%-2d  val=%-5d (0x%08h)", cdb_fu3_rob_idx, cdb_fu3_phys_reg, cdb_fu3_result, cdb_fu3_result);
        if (cdb_bpu_valid) begin
            if (cdb_bpu_mispredict)
                $display("  *** BPU MISPREDICTION ***  rob=%0d  corr_pc=0x%08h", cdb_bpu_rob_idx, cdb_bpu_correct_pc);
            else
                $display("  BPU: rob=%0d  correct prediction  corr_pc=0x%08h", cdb_bpu_rob_idx, cdb_bpu_correct_pc);
        end
    ...
    initial begin
        ...
        // Per-cycle trace for 80 cycles (3 flushes need time)
        repeat (80) begin
            @(posedge clk); #1;
            print_cycle(cyc);
            cyc = cyc + 1;
        ...
        // ============================================================
        $display("############################################################");
        $display("# COMMITTED RRAT  (arch -> phys)");
        $display("############################################################");
        $write("  ");
        for (i = 0; i < 32; i = i + 1) begin
            $write("x%-2d->p%-2d  ", i, dbg_arf_rat[i*6 +: 6]);
            if (i % 8 == 7) begin $display(""); if (i < 31) $write("  "); end
        end
        ...
        $display("  --- Setup regs (words 0-3) ---");
        check_reg(1,  32'd10,  "ADDI x1=10");
        check_reg(2,  32'd20,  "ADDI x2=20");
        check_reg(3,  32'd10,  "ADDI x3=10");
        check_reg(4,  32'd99,  "ADDI x4=99");

        $display("  --- After BRANCH 1 mispred flush (BEQ x1,x2 NOT taken) ---");
        check_reg(5,  32'd11,  "ADDI x5=11  (post-flush correct path)");
        check_reg(6,  32'd22,  "ADDI x6=22  (post-flush correct path)");

        $display("  --- After BRANCH 2 mispred flush (BNE x1,x3 NOT taken) ---");
        check_reg(7,  32'd33,  "ADD  x7=x5+x6=33  (RAW on post-flush regs)");

        $display("  --- BRANCH 3 correct pred (BEQ x1,x3 TAKEN, skip w10) ---");
        check_reg(8,  32'd0,   "x8=0  (SQUASHED: ADDI x8,-1107 never executed)");
        check_reg(9,  32'd33,  "ADDI x9=33  (branch target)");

        $display("  --- After BRANCH 4 mispred flush (BEQ x2,x4 NOT taken) ---");
        check_reg(10, 32'd66,  "ADD  x10=x7+x9=66  (RAW after 3rd flush)");
        check_reg(11, 32'd42,  "ADDI x11=42");
        check_reg(12, 32'd108, "ADD  x12=x10+x11=108  (deep post-flush RAW)");
